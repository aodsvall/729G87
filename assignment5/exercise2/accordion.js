  ...
  connectedCallback() {
    this.shadowRoot.querySelectorAll("button").forEach((button) => {
      button.addEventListener("click", () => {
        ...
        const ulChildren = this.shadowRoot.querySelector("ul").children.length;
        for (let i = 0; i < ulChildren; i++) {
          this.shadowRoot
            .querySelector("ul")
            .children[i].classList.remove("active");
          this.shadowRoot
            .querySelector("ul")
            .children[i].children[0].setAttribute("aria-expanded", "false");
          this.shadowRoot.querySelector("ul").children[
            i
